...
export function drawQrcode2d(canvas, options = {}) {
  if (!canvas) return;
  ...
  const qrcode = new QRCode(options.typeNumber || -1, options.correctLevel ?? QRErrorCorrectLevel.M);
  qrcode.addData(utf16to8(String(options.text || '')));
  qrcode.make();

  const moduleCount = qrcode.getModuleCount();
  const tileW = width / moduleCount;
  const tileH = height / moduleCount;
