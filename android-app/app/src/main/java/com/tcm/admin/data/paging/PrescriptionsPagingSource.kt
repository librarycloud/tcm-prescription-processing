...
class PrescriptionsPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, JSONObject> {
        ...
        return try {
            val response = apiClient.prescriptionsPaged(status, keyword, storeId, doctorId, page)
            val itemsArray = response.optJSONArray("list")
            val items = buildList {
                for (i in 0 until (itemsArray?.length() ?: 0)) {
                    itemsArray?.optJSONObject(i)?.let { add(it) }
                }
            }
