...
可将配置链接通过微信、钉钉、企微、短信或备忘录直接发给员工，点击链接即可**自动唤起 App、完成配置并持久化保存**：

| 链接格式 | 示例 | 适用场景 |
| --- | --- | --- |
| **标准格式（推荐）** | `tcmadmin://config?server=https://api.tcm.yourdomain.com` | 生产线上域名 |
| **局域网真机** | `tcmadmin://config?server=http://192.168.1.100:3000` | 药店内部 Wi-Fi 联调 |
| **超简短格式** | `tcm://config?server=http://192.168.1.100:3000` | 便于短信下发 |

> 💡 参数名兼容 `server`、`url`、`baseURL` 与 `api`（不区分大小写）；亦支持简写格式如 `tcmadmin://192.168.1.100:3000`。

#### 3. 二维码扫码导入（系统相机直接支持）
将上述任意导入链接生成为标准二维码（例如用草料二维码）：
1. 员工打开 iPhone **系统相机**对准该二维码。
2. 画面中会自动出现 **“在「药房助手」中打开”** 的黄色提示胶囊。
3. 点击即可启动 App 并自动导入切换服务器，无需任何手工输入！
