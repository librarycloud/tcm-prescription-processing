...
public struct LiveScannerView: View {
    ...
    public var body: some View {
                ...
                HStack {
                    ...
                    Button(action: { isTorchOn.toggle() }) {
                        Image(systemName: isTorchOn ? "flashlight.on.fill" : "flashlight.off.fill")
                            .font(.system(size: (24) * ThemeManager.shared.fontScale))
        ...
        .alert(item: $equipmentAlertData) { equip in
            Alert(
                title: Text(equip.name),
                message: Text("设备类型：\(equip.typeName ?? "通用设备")\n当前状态：\(equip.status == 1 ? "正常空闲" : "使用中")"),
                primaryButton: .default(Text("前往加工管理")) {
                    dismiss()
                    // 切换到加工管理 Tab
                },
        ...
        .alert(isPresented: Binding(get: { scanError != nil }, set: { if !$0 { scanError = nil } })) {
            Alert(
                title: Text("扫描提示"),
                message: Text(scanError ?? ""),
                dismissButton: .cancel(Text("我知道了"))
            )
        }
    }
