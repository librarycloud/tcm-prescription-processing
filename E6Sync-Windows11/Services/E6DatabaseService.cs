...
ORDER BY p.[_c_];";
            ...
            using (var command = new SqlCommand(sql, connection))
            {
                ...
                connection.Open();
                using (var reader = command.ExecuteReader()) while (reader.Read()) { result.Products.Add(new E6PharmacyProductUpload { productCode = Convert.ToString(reader["编号"])?.Trim(), name = Convert.ToString(reader["名称"])?.Trim(), category = ToNullableText(reader["分类"]), categoryCode = ToNullableText(reader["分类编号"]), barcode = ToNullableText(reader["条形码"]), specification = ToNullableText(reader["规格"]), dosageForm = ToNullableText(reader["剂型"]), manufacturer = ToNullableText(reader["生产厂商"]), categoryAttribute = ToNullableText(reader["商品类别属性"]), unit = ToNullableText(reader["单位"]), retailPrice = reader.IsDBNull(reader.GetOrdinal("零售价")) ? "0" : Convert.ToDecimal(reader["零售价"]).ToString("0.##", System.Globalization.CultureInfo.InvariantCulture), e6CreatedAt = ToIso(reader["创建日期"]), e6ModifiedAt = ToIso(reader["修改日期"]) }); result.Cursor = Convert.ToBase64String((byte[])reader["_c_"]); }
            }
            return result;
        }
        ...
        public E6PharmacyInventorySnapshot QueryPharmacyInventory(DateTime inventoryDate, string cursor, string locationCursor)
        {
            ...
            var locationBytes = DecodeCursor(locationCursor);
            var cursorClause = bytes == null && locationBytes == null ? "" : @" AND EXISTS (
    SELECT 1
    ...
    WHERE changed.[商品id] = i.[商品id]
      AND ISNULL(changed.[批号], '') = ISNULL(i.[批号], '')" +
                ((bytes == null && locationBytes == null) ? "" : " AND (" +
                    (bytes == null ? "" : "changed.[_c_] > @cursor") +
                    (bytes == null || locationBytes == null ? "" : " OR ") +
                    (locationBytes == null ? "" : "changedLocation.[_c_] > @locationCursor") + ")") + @") ";
            var sql = @"SELECT p.[编号] AS [商品编号], l.[名称] AS [货位名称], i.[批号], i.[生产日期], i.[有效期至], i.[入库时间], i.[数量], i.[_c_], l.[_c_] AS [货位_c_] FROM dbo.[AC货位商品帐] i LEFT JOIN dbo.[DC商品] p ON p.[ID] = i.[商品id] LEFT JOIN dbo.[DC货位] l ON l.[ID] = i.[货位id] WHERE i.[数量] > 0 " + cursorClause + "ORDER BY i.[_c_];";
            using (var connection = new SqlConnection(BuildConnectionString(config.PharmacyE6)))
            using (var command = new SqlCommand(sql, connection))
            {
                if (bytes != null) command.Parameters.Add("@cursor", SqlDbType.Binary, 8).Value = bytes;
                if (locationBytes != null) command.Parameters.Add("@locationCursor", SqlDbType.Binary, 8).Value = locationBytes;
                connection.Open();
                using (var reader = command.ExecuteReader()) while (reader.Read()) { result.Batches.Add(new E6PharmacyBatchUpload { productCode = ToNullableText(reader["商品编号"]), locationName = ToNullableText(reader["货位名称"]), batchNo = ToNullableText(reader["批号"]) ?? "", productionDate = ToDate(reader["生产日期"]), expiryDate = ToDate(reader["有效期至"]), inboundDate = ToDate(reader["入库时间"]), quantity = Convert.ToDecimal(reader["数量"]).ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) }); result.Cursor = MaxCursor(result.Cursor, reader["_c_"]); if (!reader.IsDBNull(reader.GetOrdinal("货位_c_"))) result.LocationCursor = MaxCursor(result.LocationCursor, reader["货位_c_"]); }
            }
            return result;
        }
        ...
        private static string MaxCursor(string current, object value) { if (value == null || value == DBNull.Value) return current; var candidate = (byte[])value; if (string.IsNullOrWhiteSpace(current)) return Convert.ToBase64String(candidate); var existing = Convert.FromBase64String(current); for (var i = 0; i < candidate.Length; i++) { if (candidate[i] == existing[i]) continue; return candidate[i] > existing[i] ? Convert.ToBase64String(candidate) : current; } return current; }
