...
namespace E6Sync.Services
{
    ...
    public sealed class E6SyncService
    {
        ...
        {
            DateTime? modifiedAfter = null; DateTime parsed;
            ...
            if (products.Count > 0) { var result = await api.SendPharmacyProductsAsync(products, cancellationToken).ConfigureAwait(false); if (!result.Success) throw new InvalidOperationException(result.Message); var latest = products[products.Count - 1].e6ModifiedAt; if (!string.IsNullOrWhiteSpace(latest)) config.Sync.LastPharmacyProductModifiedAt = latest; }
