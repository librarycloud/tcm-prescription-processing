...
namespace E6Sync.Services
{
    public sealed class ApiService : IDisposable
    {
        ...
        private async Task<ApiResult> PostPharmacyAsync(string path, object body, CancellationToken cancellationToken)
        {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, endpoint))
            {
                request.Headers.Add("X-API-Key", config.ApiKey); request.Content = new StringContent(serializer.Serialize(body), Encoding.UTF8, "application/json");
                try { using (var response = await client.SendAsync(request, HttpCompletionOption.ResponseContentRead, cancellationToken).ConfigureAwait(false)) { var text = await response.Content.ReadAsStringAsync().ConfigureAwait(false); ApiResponse payload = null; try { payload = serializer.Deserialize<ApiResponse>(text); } catch { } if (!response.IsSuccessStatusCode || payload == null || payload.code != 0) return new ApiResult { HttpStatus = (int)response.StatusCode, Retryable = (int)response.StatusCode == 429 || (int)response.StatusCode >= 500, Message = payload == null ? "API 响应无效" : payload.message ?? "上传失败" }; return new ApiResult { Success = true, HttpStatus = (int)response.StatusCode, Message = payload.message ?? "上传成功" }; } }
                catch (TaskCanceledException) { if (cancellationToken.IsCancellationRequested) throw; return new ApiResult { Retryable = true, Message = "网络超时" }; }
