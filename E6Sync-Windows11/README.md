...
程序启动后自动定时器运行。首次 `lastSyncTime` 为空时，程序不会自动全量查询，只会提示先执行一次手动同步。首次手动同步不修改 `lastSyncTime`，因此在手动同步确认无误后，请在 `config.json` 中设置一个合适的初始时间，或等待自动模式拥有已有的 `lastSyncTime` 后运行。正常自动同步会查询 `lastSyncTime - 2 分钟` 到当前时间；只有整轮没有失败，才会将 `lastSyncTime` 前移。失败后会在下一轮重新查询，API 的 `storeCode + externalOrderNo` 幂等规则可处理边界重复。
