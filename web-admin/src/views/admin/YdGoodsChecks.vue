  ...
  <div class="page yd-check-page">
    ...
    <el-card v-if="selectedCheck" class="detail-card" shadow="never">
      ...
      <el-table v-loading="itemsLoading" :data="items" border table-layout="auto">
        ...
        <el-table-column label="操作" min-width="240" fixed="right"><template #default="{ row }"><el-button link type="primary" :icon="Edit" :disabled="row.reviewStatus === 1" @click="openCount(row)">{{ row.recountQty === null && row.firstCountQty !== null ? '复盘' : '盘点' }}</el-button><el-button link type="primary" @click="openLocation(row)">货位</el-button><el-button link type="success" :disabled="row.reviewStatus === 1" @click="review(row)">二次确认</el-button></template></el-table-column>
      </el-table>
...
const createDialog = ref(false); const createForm = reactive({ storeId: undefined, checkName: '', checkType: 1 }); const countDialog = ref(false); const countForm = reactive({ mode: 'initial', id: null, product: null, batchNo: '', location: '', manualBatch: false, systemQty: 0, qty: 0 }); const locationDialog = ref(false); const locationForm = reactive({ id: null, systemLocationName: '', location: '' }); const candidateDialog = ref(false); const candidates = ref([]); const candidateKeyword = ref(''); const candidateSearched = ref(false); const candidateLoading = ref(false);
...
async function loadCandidates() { const keyword = candidateKeyword.value.trim(); if (!selectedCheck.value || !keyword) { candidates.value = []; candidateSearched.value = false; return; } candidateSearched.value = true; candidateLoading.value = true; try { candidates.value = await getGoodsCheckCandidates(selectedCheck.value.id, { keyword }); } finally { candidateLoading.value = false; } }
