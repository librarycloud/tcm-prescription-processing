<template>
  <div class="page yd-check-page">
    ...
    <el-card shadow="never">
      <el-form class="filters" inline @submit.prevent="loadChecks">
    ...
    <el-card v-if="selectedCheck" class="detail-card" shadow="never">
      ...
      <el-form class="filters detail-filters" inline @submit.prevent="loadItems">
        ...
        <el-input v-model.trim="itemQuery.batchNo" clearable placeholder="批号" @keyup.enter="loadItems" />
        <el-input v-model.trim="itemQuery.locationName" clearable placeholder="货位" @keyup.enter="loadItems" />
        <el-select v-model="itemQuery.status" clearable placeholder="全部明细" @change="loadItems">
      ...
      <el-table v-loading="itemsLoading" :data="items" border table-layout="auto">
        ...
        <el-table-column label="商品名称" min-width="140"><template #default="{ row }">{{ row.product?.name || '-' }}</template></el-table-column>
        <el-table-column label="规格"><template #default="{ row }">{{ row.product?.specification || '-' }}</template></el-table-column>
    ...
    <el-dialog v-model="candidateDialog" title="选择 E6 库存批次" width="820px"><el-form class="filters" inline @submit.prevent="loadCandidates"><el-input v-model.trim="candidateKeyword" clearable placeholder="商品编号、名称或条形码" @keyup.enter="loadCandidates" /><el-button type="primary" :icon="Search" @click="loadCandidates">查询</el-button></el-form><el-table v-loading="candidateLoading" :data="candidates" border max-height="420" @row-click="chooseCandidate"><el-table-column label="商品编号"><template #default="{ row }">{{ row.product?.productCode }}</template></el-table-column><el-table-column label="商品名称"><template #default="{ row }">{{ row.product?.name }}</template></el-table-column><el-table-column label="批号" prop="batchNo" /><el-table-column label="货位"><template #default="{ row }">{{ row.locationName || '-' }}</template></el-table-column><el-table-column label="系统数量" align="right"><template #default="{ row }">{{ qty(row.quantity) }}</template></el-table-column><el-table-column label="状态"><template #default="{ row }">{{ row.counted ? '已盘' : '未盘' }}</template></el-table-column></el-table></el-dialog>
...
const checkQuery = reactive({ storeId: undefined, status: undefined }); const checkPagination = reactive({ page: 1, pageSize: 20, total: 0 }); const itemQuery = reactive({ keyword: '', batchNo: '', locationName: '', status: undefined }); const itemPagination = reactive({ page: 1, pageSize: 50, total: 0 });
...
.detail-filters { margin-bottom: 14px; }
