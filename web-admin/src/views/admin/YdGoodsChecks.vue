  ...
  <div class="page yd-check-page">
    ...
    <el-card v-if="selectedCheck" class="detail-card" shadow="never">
      ...
      <el-table v-loading="itemsLoading" :data="items" border table-layout="auto" @selection-change="handleReviewSelectionChange">
        ...
        <el-table-column label="系统货位"><template #default="{ row }">{{ row.systemLocationCode ? row.systemLocationCode + '-' : '' }}{{ row.systemLocationName || '-' }}</template></el-table-column>
        <el-table-column label="盘点货位"><template #default="{ row }">{{ row.countLocationName ? (row.countLocationCode ? row.countLocationCode + '-' : '') + row.countLocationName : '-' }}</template></el-table-column>
        ...
        <el-table-column label="状态"><template #default="{ row }">{{ statusText(row.checkStatus, row.needsAdjustment, row.reviewStatus) }}</template></el-table-column>
        <el-table-column label="操作" min-width="270" fixed="right"><template #default="{ row }"><el-button v-if="countActionText(row)" link type="primary" :icon="Edit" @click="openCount(row)">{{ countActionText(row) }}</el-button><span v-else class="muted-action">{{ row.reviewStatus === 1 && row.checkStatus === 2 ? '初盘已复核' : row.reviewStatus === 1 ? '已复核' : '待复核' }}</span><el-button v-if="userStore.isManager" link type="primary" @click="openLocation(row)">货位</el-button><el-button v-if="userStore.isManager" link type="success" :disabled="row.reviewStatus === 1 || row.firstCountQty === null" @click="review(row)">{{ row.recountQty === null && row.checkStatus === 2 ? '初盘复核' : '最终复核' }}</el-button></template></el-table-column>
      </el-table>
    ...
    <el-drawer v-model="candidateDialog" class="candidate-drawer" title="新增盘点记录" direction="rtl" size="min(1240px, 92vw)">
      ...
      <template v-if="selectedCandidateProduct">
        ...
        <el-table ref="candidateInventoryTableRef" class="candidate-table" :data="selectedCandidateInventories" border max-height="360" scrollbar-always-on table-layout="auto" style="width: 100%">
          ...
          <el-table-column label="系统货位" min-width="120"><template #default="{ row }">{{ row.systemLocationCode ? row.systemLocationCode + '-' : '' }}{{ row.systemLocationName || '-' }}</template></el-table-column>
          <el-table-column label="盘点货位" min-width="120"><template #default="{ row }">{{ row.countLocationName ? (row.countLocationCode ? row.countLocationCode + '-' : '') + row.countLocationName : '-' }}</template></el-table-column>
          ...
          <el-table-column label="状态" min-width="90"><template #default="{ row }">{{ statusText(row.checkStatus, row.needsAdjustment, row.reviewStatus) }}</template></el-table-column>
          <el-table-column label="操作" min-width="250" fixed="right"><template #default="{ row }"><el-button v-if="countActionText(row)" link type="primary" :icon="Edit" @click="openCount(row)">{{ countActionText(row) }}</el-button><span v-else class="muted-action">{{ row.reviewStatus === 1 && row.checkStatus === 2 ? '初盘已复核' : row.reviewStatus === 1 ? '已复核' : '待复核' }}</span><el-button v-if="userStore.isManager && row.id" link type="primary" @click="openLocation(row)">货位</el-button><el-button v-if="userStore.isManager && row.id" link type="success" :disabled="row.reviewStatus === 1 || row.firstCountQty === null" @click="review(row)">{{ row.recountQty === null && row.checkStatus === 2 ? '初盘复核' : '最终复核' }}</el-button></template></el-table-column>
        </el-table>
      </template>
    </el-drawer>
    <el-dialog v-model="locationDialog" title="修改盘点货位" width="400px"><el-form label-width="90px"><el-form-item label="原货位">{{ locationForm.systemLocationName || '-' }}</el-form-item><el-form-item label="盘点货位"><el-select v-model="locationForm.locationCode" filterable clearable placeholder="留空表示未变化" style="width: 100%"><el-option v-for="loc in locations" :key="loc.code" :label="`${loc.code}-${loc.name}`" :value="loc.code" /></el-select></el-form-item></el-form><template #footer><el-button @click="locationDialog = false">取消</el-button><el-button type="primary" @click="saveLocation">保存</el-button></template></el-dialog>
  </div>
</template>
...
import { getE6PharmacyCategoryMappings, getE6PharmacyLocations } from '@/api/e6Pharmacy';
...
const statusText = (value, adjustment, reviewStatus) => adjustment ? '需调整库存' : ({ 0: '未盘', 1: '待复核', 2: Number(reviewStatus) === 1 ? '待复盘' : '待复核', 3: '复盘待复核', 4: '需调整库存', 5: '新增批号', 6: '已确认' }[Number(value)] || '-');
const price = (value) => value === null || value === undefined ? '-' : `¥ ${Number(value).toFixed(2)}`;
const countActionText = (row) => {
  if (row.canEditRecount) return '修改复盘';
  if (row.firstCountQty === null) return '盘点';
  if (row.needsRecount) return '复盘';
  if (row.canEditInitial) return '修改初盘';
  return '';
};
const countDialogTitle = computed(() => `${countForm.editing ? '修改' : ''}${countForm.mode === 'recount' ? '复盘' : '初盘'}`);
...
const locations = ref([]);
async function loadLocations(storeId) { locations.value = await getE6PharmacyLocations({ storeId }); }

async function selectCheck(row) { selectedCheck.value = row; selectedReviewRows.value = []; itemPagination.page = 1; await loadLocations(row.storeId); await loadItems(); }
...
function openCount(row) { const mode = row.canEditRecount || row.needsRecount ? 'recount' : 'initial'; Object.assign(countForm, { mode, id: row.id || row.checkItemId || null, product: row.product, batchNo: row.batchNo, locationCode: row.countLocationCode || row.systemLocationCode || '', manualBatch: false, editing: mode === 'recount' ? Boolean(row.canEditRecount) : Boolean(row.canEditInitial), systemQty: mode === 'recount' ? (row.recountSystemQty ?? row.systemQty) : row.systemQty, qty: mode === 'recount' ? row.recountQty ?? row.firstCountQty ?? 0 : row.firstCountQty ?? 0 }); countDialog.value = true; }
async function saveCount() { if (countForm.mode === 'recount') await recountGoodsCheckItem(countForm.id, { recountQty: countForm.qty }); else { if (countForm.manualBatch && !countForm.batchNo.trim()) return ElMessage.warning('请输入新增批号'); await addInitialCount(selectedCheck.value.id, { ...(countForm.editing ? { itemId: countForm.id } : {}), productId: countForm.product.id, batchNo: countForm.batchNo, locationCode: countForm.locationCode || undefined, firstCountQty: countForm.qty }); } countDialog.value = false; await loadItems(); await loadChecks(); if (candidateDialog.value) await loadCandidates(true); ElMessage.success(countForm.editing ? '盘点记录已修改' : '盘点数量已保存'); }
...
function openManualBatch() { if (!selectedCandidateProduct.value) return; Object.assign(countForm, { mode: 'initial', id: null, product: selectedCandidateProduct.value, batchNo: '', locationCode: '', manualBatch: true, editing: false, systemQty: 0, qty: 0 }); countDialog.value = true; }
function openLocation(row) { Object.assign(locationForm, { id: row.id, systemLocationName: row.systemLocationCode ? `${row.systemLocationCode}-${row.systemLocationName}` : row.systemLocationName, locationCode: row.countLocationCode || row.systemLocationCode || '' }); locationDialog.value = true; }
async function saveLocation() { await updateGoodsCheckLocation(locationForm.id, { countLocationCode: locationForm.locationCode }); locationDialog.value = false; await loadItems(); if (candidateDialog.value) await loadCandidates(true); ElMessage.success('货位已保存'); }
