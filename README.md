...
```mermaid
flowchart TD
    E6[(外部 E6 诊所 / 药店库)] -- 只读同步 --> E6Sync[E6Sync 同步工具 Win]
    E6Sync -- HTTP POST API Key --> API

    WebAdmin[Web 管理端 PC<br>Vue 3 + Vite] --> API
    ...
    API --> Notif[多渠道通知矩阵<br>短信/邮件/群机器]
    API --> Storage[(本地文件持久化<br>处方附件/调配凭证)]
```
