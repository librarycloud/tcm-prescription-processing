...
export function requirePackageManager(actor) {
  if (!isManager(actor)) throw new AppError("无包裹管理权限", 403);
  if (isStoreAdmin(actor) && !Number(actor.storeId)) {
    throw new AppError("门店管理员未绑定门店", 403);
  }
}

export function packageAccessWhere(actor) {
  requirePackageManager(actor);
