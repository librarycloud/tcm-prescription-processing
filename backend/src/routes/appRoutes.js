...
export default async function appRoutes(fastify) {
  ...
  fastify.get('/version/:platform', async (request, reply) => {
    ...
    const rawPlatform = request.params.platform || 'android';
    // Sanitize platform to prevent SSRF and path traversal (CodeQL js/request-forgery)
    const platform = /^[a-zA-Z0-9_-]+$/.test(rawPlatform) ? rawPlatform : 'android';
  ...
  fastify.get('/releases/:filename', async (request, reply) => {
    ...
    const filename = encodeURIComponent(String(request.params.filename || ''));
    if (hubUrl && appId) {
